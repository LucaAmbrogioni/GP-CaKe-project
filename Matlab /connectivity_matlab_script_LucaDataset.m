...
theta_MEG = ct_transform(cfg_k, cfg_param, mode);
[K_tot K] = ct_kernel(cfg_k, theta_MEG, T_x, T_y, 0);
K_tot = max_c_1*K_tot;
K = max_c_1*K;
...
for trial = 1:number_trials
    ...
    mean_pos{trial} = MEG_signal{trial}(1:2,:)*inv_K';
...
%% EMG Covariance function plot
theta_EMG = ct_transform(cfg_k, cfg_param, mode);
[K_tot K] = ct_kernel(cfg_k, theta_EMG, T_x, T_y, 0);
K_tot = max_c_1*K_tot;
K = max_c_1*K;
...
% 
% 
% MEG_labels = data.label(1:2);
% EMG_labels = data.label(3:4);
% MEG_signal = MEG_mean_process;
% EMG_signal = EMG_mean_process;
% parameter_labels = {'amplitude', 'relaxation', 'frequency'};
% MEG_parameters = theta_MEG{3};
% EMG_parameters = theta_EMG{2};
% 
% save('connectivity_MEG_labels', 'MEG_labels')
% save('connectivity_EMG_labels', 'EMG_labels')
% save('connectivity_MEG_signal', 'MEG_signal')
% save('connectivity_EMG_signal', 'EMG_signal')
% save('connectivity_parameter_labels', 'parameter_labels')
% save('connectivity_MEG_parameters', 'MEG_parameters')
% save('connectivity_EMG_parameters', 'EMG_parameters')
